...
void initializeMotor() {
  ...
  do {
    ...
    if(sright) {  // LEFT
      delay(1000); //JUST ADDED
      stopMotor();
      //delay(1000); //JUST REMOVED

      //motorStep(true, SENSOR_BACK_STEP); // Rotate 8 degrees CW, RnW
      break;
    }
    else if(sleft) {  // LEFT
      ...
      //delay(_step_delay); //JUST REMOVED
      //stepOverCheck = 500000; //JUST REMOVED
      do {
        motorStep(true, 1); // Rotate CW until right sensor is interrupted,, L->R
        //Removed stepOverCheck, RnW
        //stepOverCheck--;
        //if(stepOverCheck < 0) {
